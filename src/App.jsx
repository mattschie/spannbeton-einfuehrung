...
const CONCRETE_CLASSES = [
  { label: "C20/25", fck: 20, Ecm: 30000 },
  { label: "C25/30", fck: 25, Ecm: 31000 },
  { label: "C30/37", fck: 30, Ecm: 33000 },
  { label: "C35/45", fck: 35, Ecm: 34000 },
  { label: "C40/50", fck: 40, Ecm: 35000 },
  { label: "C45/55", fck: 45, Ecm: 36000 },
  { label: "C50/60", fck: 50, Ecm: 37000 },
];
const ES = 200000; // N/mm², E-Modul Betonstahl

// Löst die Nulllinientiefe x (von der Druckkante) für ein gerissenes (Zustand-II-)
// Rechteckprofil per Bisektion: K·x²·(d−x/3)/(d−x) = Mabout, K = 0,5·Ecm·εs·b
function solveNeutralAxis(Mabout, d, K) {
  if (Mabout <= 0 || d <= 0) return 0;
  const f = (x) => (K * x * x * (d - x / 3)) / (d - x) - Mabout;
  let lo = 1e-6, hi = d - 1e-6;
  if (f(hi) < 0) return hi; // Grenzfall: selbst maximale Zugzone reicht nicht (sehr großes M)
  for (let i = 0; i < 60; i++) {
    const mid = (lo + hi) / 2;
...
export default function App() {
  ...
  const [fckIdx, setFckIdx] = useState(2);
  const [P0, setP0] = useState(0);
  const [loss, setLoss] = useState(0);
  const [eA, setEA] = useState(0);
  const [eF, setEF] = useState(0);
  const [eLinked, setELinked] = useState(true);
  const [qAdd, setQAdd] = useState(19);
  const [d1, setD1] = useState(7.5);
  const [L, setL] = useState(12.5);
  const [xSel, setXSel] = useState(6.25);
  ...
  const Ecm = CONCRETE_CLASSES[fckIdx].Ecm;
  ...
  const geo = useMemo(() => {
    ...
    const computeAt = (x) => {
      ...
      // Erforderliche Bewehrung über Dehnungsgleichgewicht im gerissenen Zustand II:
      // Beton im Druckbereich elastisch (Dreiecksblock, E_cm), Betonstahl bei Fließen
      // (ε_s = f_yd/E_s). Lineare Dehnungsebene koppelt Nulllinie x und Randdehnung.
      // P(x) geht als äußere Kraft ein, e(x) über das Versatzmoment beim Bezug auf die
      // Bewehrungslage (Mabout) – und jetzt zusätzlich E_cm über K und die Nulllinie x.
      const epsS = FYD / ES;
      const d = hMm - d1Mm; // Nutzhöhe (Bewehrungslage ↔ gegenüberliegende Druckkante)
      const K = 0.5 * Ecm * epsS * bMm;
      let asTopMm2 = 0, asBottomMm2 = 0, xNaTop = 0, xNaBottom = 0;
      ...
      } else if (sigmaBottom < 0) {
        const Mabout = (MqNmm - MpNmm) + Px * (d - hMm / 2); // N*mm, um Bewehrungslage unten
        const xNA = solveNeutralAxis(Mabout, d, K);
        xNaBottom = xNA;
        if (xNA > 0) {
          const C = (K * xNA * xNA) / (d - xNA);
          asBottomMm2 = Math.max(0, (C - Px) / FYD);
        }
      } else if (sigmaTop < 0) {
        const Mabout = -(MqNmm - MpNmm) + Px * (d - hMm / 2); // gespiegelt für Zug oben
        const xNA = solveNeutralAxis(Mabout, d, K);
        xNaTop = xNA;
        if (xNA > 0) {
          const C = (K * xNA * xNA) / (d - xNA);
          asTopMm2 = Math.max(0, (C - Px) / FYD);
        }
      }
      ...
      return {
        ...
        sigmaTop, sigmaBottom, asTopMm2, asBottomMm2, xNaTop, xNaBottom,
      };
    };
  ...
  }, [b, h, fck, Ecm, P0, loss, eA, eF, qAdd, d1, L]);
    ...
    <div className="app">
      ...
      <header>
        ...
        <p>
          ...
          Achtung: Es handelt sich hier um Näherungen, dies ist nicht für die Bemessung geeignet!
        </p>
      </header>
        ...
        <div>
          <div className="panel">
            ...
            <div className="param">
              ...
              <div className="static-readout" style={{ margin: "8px 0 0" }}>
                E_cm = {de(Ecm / 1000, 1)} · 10³ N/mm² &nbsp;·&nbsp; n = E_s/E_cm = {de(ES / Ecm, 1)}
              </div>
            </div>
            {geo.coverWarning && (
            ...
            <div className="hero-grid">
              ...
              <div>
                ...
                <div className="readout-grid">
                  ...
                  <span className="k">As,erf oben</span><span>{cur.sigmaTop >= 0 ? "– (kein Zug)" : `${de(asTopCm2, 2)} cm²`}</span>
                  <span className="k">As,erf unten</span><span>{cur.sigmaBottom >= 0 ? "– (kein Zug)" : `${de(asBottomCm2, 2)} cm²`}</span>
                  <span className="k">Druckzone x</span><span>{minSigma < 0 ? `${de((cur.xNaTop || cur.xNaBottom) / 10, 1)} cm` : "– (ungerissen)"}</span>
                </div>
...
Bewehrung – Dehnungsgleichgewicht, gerissener Zustand II (berücksichtigt E_cm):
ε_s = f_yd / E_s                          (Stahl fließt, Bemessungsannahme)
K = 0,5 · E_cm · ε_s · b                   d = h − d₁  (Nutzhöhe)
K · x² · (d − x/3) / (d − x) = M_um_Bewehrung     → Nulllinie x (iterativ, Bisektion)
C = K · x² / (d − x)                        As,erf = (C − P(x)) / f_yd
→ E_cm geht direkt in K und damit in die Lage der Nulllinie x ein: bei
  gleicher Last liefert ein steiferer Beton (größeres E_cm) eine andere
  Druckzonenhöhe x und damit ein anderes As,erf als ein weicherer Beton.`}
              </div>
            </details>
          </div>
        </div>
