...
export default function App() {
  ...
  const geo = useMemo(() => {
    ...
    // Erforderliche Bewehrung (vereinfachte Näherung): Zugseite trägt das nicht
    // durch Vorspannung ausgeglichene Moment M_net über den Hebelarm z = 0,9·d.
    const d1Mm = d1 * 10;
    const z = 0.9 * (hMm - 2 * d1Mm > 0 ? hMm - d1Mm : hMm * 0.8);
    const calcReinforcement = (Mnet) => {
      const threshold = 0.5; // kNm, Rauschgrenze um 0
      if (Mnet > threshold) {
        return { asBottomMm2: (Mnet * 1e6) / (z * FYD), asTopMm2: 0 };
      } else if (Mnet < -threshold) {
        return { asTopMm2: (-Mnet * 1e6) / (z * FYD), asBottomMm2: 0 };
      }
      return { asTopMm2: 0, asBottomMm2: 0 };
    };

    return { A, W, fctm, compLimit, coverWarning, computeAt, points, bMm, hMm, selfWeight, qTotal, z, d1Mm, calcReinforcement };
  }, [b, h, fck, P0, loss, eA, eF, qAdd, d1, L]);

  const cur = geo.computeAt(xClamped);
  const reinf = geo.calcReinforcement(cur.Mnet);
  const asTopCm2 = reinf.asTopMm2 / 100;
  const asBottomCm2 = reinf.asBottomMm2 / 100;
    ...
    <div className="app">
      ...
      <header>
        ...
        <p>
          ...
          Achtung: Nicht für die Bemessung geeignet!
        </p>
      </header>
      ...
      <div className="layout">
        ...
        <div>
          ...
          <div className="panel">
            ...
            <ParamSlider label="Exzentrizität Feldmitte e꜀" value={eF} min={-20} max={50} step={0.5} unit="cm" decimals={1} onChange={setEFLinked} />
            <div className="param">
              <div className="param-row">
              ...
              <button className="link-btn" onClick={toggleLink}>
                {eLinked ? "🔗 verknüpft mit e꜀ — klicken zum Entkoppeln" : "🔓 unabhängig — klicken zum Verknüpfen"}
              </button>
            </div>
          </div>
            ...
            <div className="hero-grid">
              ...
              <div>
                ...
                <div className="readout-grid">
                  ...
                  <span className="k">f꜀ₜₘ (Riss)</span><span>{de(geo.fctm, 2)} N/mm²</span>
                  <span className="k">0,6·f꜀ₖ (Druck)</span><span>{de(geo.compLimit, 2)} N/mm²</span>
                  <span className="k">P(x)</span><span>{de(cur.P, 0)} kN</span>
                  <span className="k">M_q(x)</span><span>{de(cur.Mq, 0)} kNm</span>
                  <span className="k">As,erf oben</span><span>{asTopCm2 > 0.001 ? `${de(asTopCm2, 2)} cm²` : "– (kein Zug)"}</span>
                  <span className="k">As,erf unten</span><span>{asBottomCm2 > 0.001 ? `${de(asBottomCm2, 2)} cm²` : "– (kein Zug)"}</span>
                </div>
...
{`e(x) = eₐ + (e꜀ − eₐ) · 4x(L−x)/L²        (parabolische Spanngliedführung)
...
f꜀ₜₘ ≈ 0,30 · f꜀ₖ^(2/3)                    Druckgrenze ≈ 0,6 · f꜀ₖ

Bewehrung (vereinfachte Näherung, kein voller ULS-Nachweis):
M_net = M_q(x) − M_p(x)                    z = 0,9 · (h − d₁)
As,erf = |M_net| / (z · f_yd)              f_yd = 435 N/mm² (B500)
→ zugseitig, dort wo M_net das Vorzeichen wechselt`}
              </div>
            </details>
          </div>
        </div>
